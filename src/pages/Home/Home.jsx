...
function AlbumCardSkeleton() {
  return (
    <div className={styles.albumCardSkeleton}>
      ...
      <div className={styles.skeletonLine} style={{ width: "75%", height: "13px", marginTop: "0.6rem" }} />
      <div className={styles.skeletonLine} style={{ width: "50%", height: "11px", marginTop: "0.4rem" }} />
    </div>
  );
}
...
function ArtistCardSkeleton() {
  return (
    <div className={styles.artistCardSkeleton}>
      ...
      <div className={styles.skeletonLine} style={{ width: "60%", height: "12px", marginTop: "0.6rem" }} />
      <div className={styles.skeletonLine} style={{ width: "40%", height: "10px", marginTop: "0.3rem" }} />
    </div>
  );
}
  ...
  return (
    ...
    >
      <div className={styles.albumCoverWrapper}>
        {album.avatarUrl ? (
          <img src={album.avatarUrl} alt={album.name} className={styles.albumCover} />
        ) : (
      ...
      <div className={styles.albumCardBody}>
        <p className={styles.albumCardName} title={album.name}>{album.name}</p>
        <p className={styles.albumCardMeta}>
          {album.musicDirectorName && <span>{album.musicDirectorName}</span>}
          {album.musicDirectorName && year && <span className={styles.dot}>·</span>}
          {year && <span>{year}</span>}
        </p>
      </div>
    </div>
  ...
  return (
    ...
    >
      <div className={styles.artistAvatarWrapper}>
        {artist.avatarUrl ? (
          <img src={artist.avatarUrl} alt={artist.name} className={styles.artistAvatar} />
        ) : (
      ...
      <p className={styles.artistName} title={artist.name}>{artist.name}</p>
...
// ── Quick stats for hero (authenticated) ────────────────────────────────────
function QuickStatCard({ label, value, color }) {
  return (
    <div className={styles.quickStat}>
      <span className={styles.quickStatValue} style={{ color }}>{value}</span>
      ...
      <div className={styles.featuredCover}>
        {coverUrl ? (
          ...
          <div className={styles.featuredCoverFallback}>
            {type === "album" ? <IconDisc /> : type === "artist" ? <IconMic /> : <IconMusic />}
          </div>
        )}
      </div>
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        setSongs(songsResult.status   === "fulfilled" ? songsResult.value   : []);
      ...
      <main className={styles.publicMain}>
        ...
        <section className={styles.hero}>
          <div className={styles.heroContent}>
            ...
            <h1 className={styles.heroTitle}>
              Music for <span className={styles.accent}>every moment</span>
            </h1>
          ...
          <div className={styles.heroDecor} aria-hidden="true">
            <div className={`${styles.floatCard} ${styles.floatCard1}`}>
              <div className={styles.floatCardCover} style={{ background: "linear-gradient(135deg,#1E3A8A,#3B82F6)" }} />
            ...
            <div className={`${styles.floatCard} ${styles.floatCard3}`}>
              <div className={styles.floatCardCover} style={{ background: "linear-gradient(135deg,#065f46,#34d399)" }} />
              <div className={styles.floatCardInfo}>
        ...
        <section className={styles.featuresSection}>
          <div className={styles.featuresInner}>
            ...
            <h2 className={styles.featuresSectionTitle}>Everything you need to enjoy music</h2>
            <div className={styles.featuresGrid}>
              {[
                { icon: "🎵", title: "Millions of Tracks", text: "Stream any song from an ever-growing library across every genre." },
                { icon: "🎧", title: "High-Quality Audio", text: "Experience studio-quality sound with lossless streaming up to 320 kbps." },
                { icon: "📻", title: "Curated Stations", text: "Discover new music with personalised stations powered by your taste." },
                { icon: "🔀", title: "Smart Playlists", text: "Auto-generated playlists that evolve with your listening habits." },
                { icon: "📱", title: "Listen Anywhere", text: "Seamlessly switch between phone, tablet, and desktop without missing a beat." },
                { icon: "🌐", title: "Offline Mode", text: "Download favourites and listen without an internet connection." },
              ].map((f) => (
      ...
      <main className={styles.mainContent}>
          ...
          <div className={styles.greetingInner}>
            ...
            {!loading && (
              <div className={styles.quickStats}>
                <QuickStatCard label="Songs" value={songs.length} color="#3B82F6" />
                <QuickStatCard label="Albums" value={albums.length} color="#8b5cf6" />
                <QuickStatCard label="Artists" value={artists.length} color="#34d399" />
              </div>
            )}
          </div>
        </section>
        ...
        <section className={styles.contentSection}>
          ...
          {!loading && songs.length > 10 && (
            <button className={styles.showMoreBtn} onClick={() => setShowAllSongs((v) => !v)}>
